...
struct segment{
    ...
    int query(int i , int b , int e , int l , int r){
        propagate(i , b , e) ;
        if(b > r or e < l or tree[i].mx < 0 or tree[i].mn > 0)return -1 ;
        if(l <= b and e <= r and b == e){
            return b ;
        }
        int mid = (b + e) >> 1 ;
        int x = query(i << 1 , b , mid , l , r) ;
        if(x == -1)x = query(i << 1 | 1 , mid + 1 , e , l , r) ;
        return x ;
...
int main() {
    ...
    while(t--) {
        ...
        while(q--){
            ...
            int cng = x - b[p] ;
            b[p] = x ;
            seg.upd(1 , 1 , n , p , p , -1 * cng) ;
            seg.upd(1 , 1 , n , p + 1 , n , cng) ;
            if(seg.tree[1].mx < 0 or seg.tree[1].mn > 0)puts("-1") ;
            else{
                int x ;
                if(b[1] > 0)x = seg.query(1 , 1 , n , 1 , n) ;
                else x = 1 ;
                printf("%d\n",x);
            }
        }
